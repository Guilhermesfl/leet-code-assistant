# LeetCode Progress Tracker

Track your completed problems here! Mark with ✅ when done.

---

## Problems 1-30: Building Foundations

- [x] 1. Two Sum
- [x] 2. Valid Anagram
- [x] 3. Contains Duplicate
- [ ] 4. Best Time to Buy and Sell Stock
- [ ] 5. Valid Parentheses
- [ ] 6. Merge Two Sorted Lists
- [ ] 7. Invert Binary Tree
- [ ] 8. Maximum Depth of Binary Tree
- [ ] 9. Palindrome Number
- [ ] 10. Remove Duplicates from Sorted Array
- [ ] 11. Maximum Subarray
- [ ] 12. Binary Search
- [ ] 13. Reverse Linked List
- [ ] 14. Climbing Stairs
- [ ] 15. Symmetric Tree
- [ ] 16. Sqrt(x)
- [ ] 17. Single Number
- [ ] 18. Intersection of Two Arrays II
- [ ] 19. Plus One
- [ ] 20. Move Zeroes
- [ ] 21. Product of Array Except Self
- [ ] 22. 3Sum
- [ ] 23. Majority Element
- [ ] 24. Happy Number
- [ ] 25. Linked List Cycle
- [ ] 26. Min Stack
- [ ] 27. Valid Sudoku
- [ ] 28. Same Tree
- [ ] 29. Path Sum
- [ ] 30. Diameter of Binary Tree

## Problems 31-60: Expanding Techniques

- [ ] 31. Longest Substring Without Repeating Characters
- [ ] 32. Longest Palindromic Substring
- [ ] 33. Merge Sorted Array
- [ ] 34. Pascal's Triangle
- [ ] 35. Container With Most Water
- [ ] 36. 3Sum Closest
- [ ] 37. Valid Palindrome
- [ ] 38. First Bad Version
- [ ] 39. Search in Rotated Sorted Array
- [ ] 40. Find Minimum in Rotated Sorted Array
- [ ] 41. Power of Two
- [ ] 42. Power of Three
- [ ] 43. Group Anagrams
- [ ] 44. Top K Frequent Elements
- [ ] 45. Kth Largest Element in an Array
- [ ] 46. Intersection of Two Linked Lists
- [ ] 47. Sort Colors
- [ ] 48. Merge Intervals
- [ ] 49. Insert Interval
- [ ] 50. Spiral Matrix
- [ ] 51. Set Matrix Zeroes
- [ ] 52. Missing Number
- [ ] 53. Find Peak Element
- [ ] 54. Search a 2D Matrix
- [ ] 55. Number of 1 Bits
- [ ] 56. Reverse Bits
- [ ] 57. Counting Bits
- [ ] 58. Range Sum Query - Immutable
- [ ] 59. Add Two Numbers
- [ ] 60. Remove Nth Node From End of List

## Problems 61-90: Intermediate Patterns

- [ ] 61. Rotate Image
- [ ] 62. Jump Game
- [ ] 63. Unique Paths
- [ ] 64. Minimum Path Sum
- [ ] 65. Maximum Product of Three Numbers
- [ ] 66. Daily Temperatures
- [ ] 67. Evaluate Reverse Polish Notation
- [ ] 68. Implement Queue using Stacks
- [ ] 69. Implement Stack using Queues
- [ ] 70. Binary Tree Level Order Traversal
- [ ] 71. Binary Tree Zigzag Level Order Traversal
- [ ] 72. Validate Binary Search Tree
- [ ] 73. Kth Smallest Element in a BST
- [ ] 74. Lowest Common Ancestor of a Binary Search Tree
- [ ] 75. Lowest Common Ancestor of a Binary Tree
- [ ] 76. Binary Tree Right Side View
- [ ] 77. Count Good Nodes in Binary Tree
- [ ] 78. Construct Binary Tree from Preorder and Inorder Traversal
- [ ] 79. Subtree of Another Tree
- [ ] 80. Number of Islands
- [ ] 81. Clone Graph
- [ ] 82. Pacific Atlantic Water Flow
- [ ] 83. Course Schedule
- [ ] 84. Course Schedule II
- [ ] 85. Number of Connected Components in an Undirected Graph
- [ ] 86. Graph Valid Tree
- [ ] 87. Word Search
- [ ] 88. Permutations
- [ ] 89. Subsets
- [ ] 90. Combination Sum

## Problems 91-120: Advanced Challenges

- [ ] 91. Letter Combinations of a Phone Number
- [ ] 92. Palindrome Partitioning
- [ ] 93. Word Break
- [ ] 94. Decode Ways
- [ ] 95. Coin Change
- [ ] 96. House Robber
- [ ] 97. House Robber II
- [ ] 98. Longest Increasing Subsequence
- [ ] 99. Partition Equal Subset Sum
- [ ] 100. Longest Common Subsequence
- [ ] 101. Edit Distance
- [ ] 102. Maximum Product Subarray
- [ ] 103. Unique Paths II
- [ ] 104. Jump Game II
- [ ] 105. Isomorphic Strings
- [ ] 106. Word Pattern
- [ ] 107. Implement Trie (Prefix Tree)
- [ ] 108. Design Add and Search Words Data Structure
- [ ] 109. Word Search II
- [ ] 110. Find Median from Data Stream
- [ ] 111. Kth Largest Element in a Stream
- [ ] 112. Task Scheduler
- [ ] 113. Longest Repeating Character Replacement
- [ ] 114. Minimum Window Substring
- [ ] 115. Sliding Window Maximum
- [ ] 116. Maximum Sliding Window
- [ ] 117. Find All Anagrams in a String
- [ ] 118. Permutation in String
- [ ] 119. Encode and Decode Strings
- [ ] 120. Design HashSet

## Problems 121-150: Expert Level

- [ ] 121. Design HashMap
- [ ] 122. LRU Cache
- [ ] 123. Copy List with Random Pointer
- [ ] 124. Reorder List
- [ ] 125. Merge k Sorted Lists
- [ ] 126. Reverse Nodes in k-Group
- [ ] 127. Rotate List
- [ ] 128. Swap Nodes in Pairs
- [ ] 129. Odd Even Linked List
- [ ] 130. Design Linked List
- [ ] 131. Maximum Binary Tree
- [ ] 132. Serialize and Deserialize Binary Tree
- [ ] 133. Binary Tree Maximum Path Sum
- [ ] 134. Binary Search Tree Iterator
- [ ] 135. Flatten Binary Tree to Linked List
- [ ] 136. Populating Next Right Pointers in Each Node
- [ ] 137. Sum Root to Leaf Numbers
- [ ] 138. Path Sum II
- [ ] 139. Path Sum III
- [ ] 140. All Nodes Distance K in Binary Tree
- [ ] 141. Surrounded Regions
- [ ] 142. Rotting Oranges
- [ ] 143. Walls and Gates
- [ ] 144. Alien Dictionary
- [ ] 145. Longest Consecutive Sequence
- [ ] 146. Meeting Rooms II
- [ ] 147. Non-overlapping Intervals
- [ ] 148. Trapping Rain Water
- [ ] 149. Largest Rectangle in Histogram
- [ ] 150. Word Ladder

---

## Progress Summary

**Completed**: 3 / 150 (2%)

Keep going! 🚀
